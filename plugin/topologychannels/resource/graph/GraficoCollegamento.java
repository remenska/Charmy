...
public class GraficoCollegamento 
	extends GraficoLink 
	 ...
	 */
	private ElementoProcessoStato elementoBox_start;
	private ElementoProcessoStato elementoBox_end;
	...
	public GraficoCollegamento(
			ElementoProcessoStato box1, 
			ElementoProcessoStato box2, 
			int i,
			boolean flusso,
			ElementoBase update)
	{
	...
	private void calcolaStartEnd(int posizione){
		...
		ystart = elementoBox_start.getPointMiddle().y;
		if ((elementoBox_start.getPointMiddle().x == elementoBox_end.getPointMiddle().x)
			...
			xend = elementoBox_end.getPointMiddle().x  + 5;
			yend = elementoBox_end.getPointMiddle().y + 5;
		}
		else{
			xend = elementoBox_end.getPointMiddle().x;
			yend = elementoBox_end.getPointMiddle().y;
		}
		if ((posizione%2)==0){
