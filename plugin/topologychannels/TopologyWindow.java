...
public  class TopologyWindow
	...
	implements IListaCanaleListener, IMainTabPanel,IHostWindow,IListaProcessoListener{
	...
	private ElementoProcesso currentProcess;
	...
	public DeclaredHost[] initHost() {
		...
		try {
			Class clazz = this.plugDataWin.getPlugManager().getGeneralClassLoader().loadClass("plugin.topologychannels.TopologyEditor");
		...
		dh[0]= new DeclaredHost(vClassEditor,"plugin.topologychannels.topologyeditor");
	...
	public void processoAdded(AddEPEvento event) {
		...
		currentProcess =
			new ElementoProcesso(
					event.getElementoProcesso().getPointMiddle(),				
				ElementoProcesso.GLOBALE,
				event.getElementoProcesso().getTipo(),
				event.getElementoProcesso().isDummy(),
				"process" + (ElementoProcesso.getNumIstanze() + 1));
		currentProcess.setName(event.getElementoProcesso().getName());
		currentProcess.getGrafico().setWidth(event.getElementoProcesso().getGrafico().getWidth());
		currentProcess.getGrafico().setHeight(event.getElementoProcesso().getGrafico().getHeight());
		currentProcess.getGrafico().setLineWeight(event.getElementoProcesso().getGrafico().getLineWeight());
		currentProcess.getGrafico().setBackgroundColor(event.getElementoProcesso().getGrafico().getBackgroundColor());
		currentProcess.getGrafico().setLineColor(event.getElementoProcesso().getGrafico().getLineColor());
		currentProcess.setId(event.getElementoProcesso().getId());
		//currentProcess.getGrafico().setX(event.getElementoProcesso().getGrafico().getX());
		//currentProcess.getGrafico().setY(event.getElementoProcesso().getGrafico().getY());
		...
		plugDataWin.getStatusBar().setText(
			"Process "
				...
				+ " inserted.");
		plugData.getListaProcesso().addElement(currentProcess);
		repaint();
	...
	public void processoRemoved(RemoveEPEvento event) {
		currentProcess=(ElementoProcesso)plugData.getListaProcesso().getElement(event.getElementoProcesso().getName());
		if(currentProcess!=null){
			plugData.getListaProcesso().removeElement(currentProcess);
		}
		//currentProcess.setName(event.getElementoProcesso().getName());
		//currentProcess.getGrafico().setWidth(event.getElementoProcesso().getGrafico().getWidth());
		//currentProcess.getGrafico().setHeight(event.getElementoProcesso().getGrafico().getHeight());
...
//		System.out.println(getListaDeiCanali().size());
//		
//		for(int i=0;i<getListaDeiCanali().size();i++){
//			System.out.println(getListaDeiCanali().getElement(i).getName()+" da:"+
//					getListaDeiCanali().getElement(i).getElementFrom().getName()+
//					" a:"+getListaDeiCanali().getElement(i).getElementTo().getName());
//		}
//		
		getListaDeiCanali().removeAllLink(currentProcess);
		

	}

	public void processoUpdate(UpdateEPEvento event) {
		currentProcess=(ElementoProcesso)plugData.getListaProcesso().getElement(event.getOldElementoProcesso().getName());
		if(currentProcess!=null){
			currentProcess.setName(event.getNewElementoProcesso().getName());
			currentProcess.setPoint(event.getNewElementoProcesso().getPointMiddle());
			currentProcess.setDummy(event.getNewElementoProcesso().isDummy());
			currentProcess.setTipo(event.getNewElementoProcesso().getTipo());
		}
		currentProcess.setName(event.getNewElementoProcesso().getName());
		currentProcess.getGrafico().setWidth(event.getNewElementoProcesso().getGrafico().getWidth());
		currentProcess.getGrafico().setHeight(event.getNewElementoProcesso().getGrafico().getHeight());
		currentProcess.getGrafico().setLineWeight(event.getNewElementoProcesso().getGrafico().getLineWeight());
		currentProcess.getGrafico().setBackgroundColor(event.getNewElementoProcesso().getGrafico().getBackgroundColor());
		currentProcess.getGrafico().setLineColor(event.getNewElementoProcesso().getGrafico().getLineColor());
		currentProcess.setId(event.getNewElementoProcesso().getId());
		getListaDeiCanali().updateListaCanalePosizione(currentProcess);

		//currentProcess.getGrafico().setX(event.getNewElementoProcesso().getGrafico().getX());
		//currentProcess.getGrafico().setY(event.getNewElementoProcesso().getGrafico().getY());
