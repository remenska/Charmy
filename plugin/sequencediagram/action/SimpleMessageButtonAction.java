...
/** Gestione dell'azione di inserimento di uno o più messaggi di tipo 
...
public class SimpleMessageButtonAction extends MouseAdapter implements MouseListener 
{
    ...
    public SimpleMessageButtonAction(SequenceWindow clwnd, SequenceToolBar tbar, AbstractButton pls)
    {
        Finestra = clwnd;
        rifToolBar = tbar;
        pulsante = pls;
        ...
        {           
            if (numClick == 1)
            {
                pulsante.setBorder(BorderFactory.createBevelBorder(1));
                  Finestra.setWindowStatus(SequenceEditor.INSERIMENTO_MESSAGGIO,ElementoMessaggio.SYNCHRONOUS,3,false);            
            }
            else
            {
            	// Doppio click => Blocco del pulsante.
                  pulsante.setBorder(BorderFactory.createBevelBorder(1,Color.LIGHT_GRAY,Color.GRAY));
                  Finestra.setWindowStatus(SequenceEditor.INSERIMENTO_MESSAGGIO,ElementoMessaggio.SYNCHRONOUS,3,true);            
            }
        }
        else
        {
