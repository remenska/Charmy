...
/** Gestione dell'azione di inserimento di uno o più loop di tipo 
	Simple nel Sequence Diagram alla pressione del relativo pulsante_
...
public class LoopMessageButtonAction extends MouseAdapter implements MouseListener 
{
    ...
    public LoopMessageButtonAction(SequenceWindow clwnd, SequenceToolBar tbar, AbstractButton pls)
    {
        Finestra = clwnd;
        rifToolBar = tbar;
        pulsante = pls;
        ...
        {           
            if (numClick == 1)
            {
                pulsante.setBorder(BorderFactory.createBevelBorder(1));
                  Finestra.setWindowStatus(SequenceEditor.DISEGNA_MESSAGGIO,ElementoMessaggio.SYNCHRONOUS,3,false);            
            }
            else
            {
            	// Doppio click => Blocco del pulsante.
                pulsante.setBorder(BorderFactory.createBevelBorder(1,Color.LIGHT_GRAY,Color.GRAY));
                  Finestra.setWindowStatus(SequenceEditor.DISEGNA_MESSAGGIO,ElementoMessaggio.SYNCHRONOUS,3,true);            
            }
        }
        else
        {
        	// Pressione del tasto destro del mouse => Sblocco pulsante.
//            Finestra.setWindowStatus(SequenceEditor.ATTESA,ElementoMessaggio.SIMPLE,false);                
              Finestra.setWindowStatus(SequenceEditor.ATTESA,ElementoMessaggio.SYNCHRONOUS,3,false);                
        }
