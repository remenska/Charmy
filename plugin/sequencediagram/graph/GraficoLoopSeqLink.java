...
public class GraficoLoopSeqLink extends GraficoLink implements Serializable
{   
	...
	private ElementoBox elementoBox; //box di partenza
	
	private ElementoTime time1; //box di partenza
	private ElementoTime time2; //box di partenza


	/**
	 * Costruttore
	 * @param box1 ElementoBox di partenza
	 * @param i posizione arco
	 * @param flusso
	 * @param update
	 */
	public GraficoLoopSeqLink(ElementoBox box1,
			ElementoTime time1, 
			int i, 
			boolean flusso, 
			ElementoBase update)
	...
	private void calcolaStartEnd(int posizione){
		...
		ystart = time1.getMinY() +  ElementoTime.hfascia;
		semiw = ElementoSeqLink.rloop;
		semih = ElementoSeqLink.rloop;;
   ...
   /** Permette di aggiornare le propriet? grafiche del testo. */
        ...
        String nuovo_testofont, int nuovo_fontstile, int nuova_fontdimensione)
	{
		...
		if (nuova_guardia!="")
			if(nuove_operazioni!="")
				testo = nuova_guardia+"/"+update.getViewName() +"/"+nuove_operazioni;
			else
				testo = nuova_guardia+"/"+update.getViewName();
		else
			if(nuove_operazioni!="")
				testo = update.getViewName() +"/"+nuove_operazioni;
			else
				testo = update.getViewName();
		testocolore = nuovo_testocolore;
		testofont = nuovo_testofont;
		fontstile = nuovo_fontstile;
		fontdimensione = nuova_fontdimensione;
