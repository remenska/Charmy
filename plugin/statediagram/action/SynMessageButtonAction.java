...
/** Gestione dell'azione di inserimento di uno o più messaggi di tipo 
	Synchronous nello State Diagram alla pressione del relativo pulsante_
...
public class SynMessageButtonAction extends MouseAdapter implements MouseListener 
{
    ...
    public SynMessageButtonAction(StateWindow clwnd, StateToolBar tbar, AbstractButton pls)
    {
        Finestra = clwnd;
        rifToolBar = tbar;
        pulsante = pls;
    ...
    public void mouseReleased(MouseEvent evt) 
    {
        ...
        {           
            if (numClick == 1)
            {
                ...
                Finestra.setWindowStatus(ThreadEditor.INSERIMENTO_MESSAGGIO,ElementoMessaggio.SYNCHRONOUS,false);            
            }
            else
            {
            	// Doppio click => Blocco del pulsante.
                pulsante.setBorder(BorderFactory.createBevelBorder(1,Color.LIGHT_GRAY,Color.GRAY));
                Finestra.setWindowStatus(ThreadEditor.INSERIMENTO_MESSAGGIO,ElementoMessaggio.SYNCHRONOUS,true);            
            }
        }
        else
        {
