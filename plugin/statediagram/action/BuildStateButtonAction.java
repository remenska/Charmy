...
/**
	Gestione dell'azione di inserimento di uno o più stati "BUILD" nello
...
public class BuildStateButtonAction extends MouseAdapter 
{
    ...
    public BuildStateButtonAction(StateWindow clwnd, StateToolBar tbar, AbstractButton pls)
    {
        Finestra = clwnd;
        rifToolBar = tbar;
        pulsante = pls;
    ...
    public void mouseReleased(MouseEvent evt) 
    {
        ...
        if (SwingUtilities.isLeftMouseButton(evt))
        {
            ...
            {      
                ...
                Finestra.setWindowStatus(ThreadEditor.DISEGNA_STATO,ElementoStato.BUILD,false);
            }
            else
            {
            	// Doppio click => Blocco del pulsante.
                pulsante.setBorder(BorderFactory.createBevelBorder(1,Color.LIGHT_GRAY,Color.GRAY));
                Finestra.setWindowStatus(ThreadEditor.DISEGNA_STATO,ElementoStato.BUILD,true);
            }        
        }
        else
        {
        	// Pressione del tasto destro del mouse => Sblocco pulsante.
        	Finestra.setWindowStatus(ThreadEditor.ATTESA,ElementoStato.BUILD,false);
        }
