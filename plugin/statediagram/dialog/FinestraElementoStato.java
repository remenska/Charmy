...
import plugin.statediagram.controllo.ControlloNomeStato;
...
import plugin.topologydiagram.utility.JComboBoxModality;
...
/** La classe realizza un pannello di dialogo tramite il quale l'utente
	pu? impostare le caratteristiche di un oggetto 'ElementoBoxTesto'_
	E' usato per settare le propriet? grafiche degli oggetti
	'GraficoTipoProcesso', 'GraficoTipoStore', 'GraficoTipoStart',
	'GraficoTipoBuild' e 'GraficoTipoEnd'. */

public class FinestraElementoStato extends JDialog
{
    ...
    private ElementoStato processo;
    ...
    private JTextArea onEntryTextArea;
    
    private JTextArea onExitTextArea;

    
    /**
     * variabile aggiunta per separare il treadEditor
     */
    private ListaStato listaStato;
    
    private ThreadEditor thrEditor;
    ...
    */
    public FinestraElementoStato(ListaStato listaStato, ElementoStato ep, Graphics2D g2d, Frame lframe, String titolo, ThreadEditor thrEditor, PlugDataManager pm)
    {
        ...
        nuovostringapannello.setLayout(new BorderLayout());        
        JPanel nameJPanel =new JPanel();
        nameJPanel.setLayout(new GridLayout(1,2));
        nameJPanel.add(new JLabel("Name:    "));
        nuovocontrollotesto = new JTextField(ep.getName(),20);
        nameJPanel.add(nuovocontrollotesto);
        
        JPanel actionJPanel = new JPanel();
        actionJPanel.setLayout(new GridLayout(1,1));
        JPanel onEntryJPanel = new JPanel();
        onEntryJPanel.setBorder(BorderFactory.createTitledBorder("On Entry Actions: "));
        if(processo.getOnEntryCode()!=null){
	        if (!processo.getOnEntryCode().equals(""))
	            onEntryTextArea = new JTextArea(processo.getOnEntryCode(),8,30);
	        else
	            onEntryTextArea = new JTextArea(8,30);
        }
        else
			onEntryTextArea = new JTextArea(8,30);
        JScrollPane panelScroll = new JScrollPane(onEntryTextArea);
        onEntryJPanel.add(panelScroll);
        actionJPanel.add(onEntryJPanel);

        nuovostringapannello.add(nameJPanel,BorderLayout.NORTH);
        nuovostringapannello.add(actionJPanel,BorderLayout.CENTER);
        ...
        processopannello.setBorder(BorderFactory.createTitledBorder("State"));			
        ...
        public void actionPerformed(ActionEvent e)
        {
            ...
            try{
                ...
                if (target.equals(annullapulsante))
                	...
                	if ((new ControlloNomeStato(pm)).checkOK(listaStato, processo.getName(),nuovocontrollotesto.getText()))
                		check = true;
                	else{
                		str =  	"You must change the state name. \n\n" + 
                    ...
                    if ((Integer.parseInt(controllorettangoloX.getText())<0)||(Integer.parseInt(controllorettangoloY.getText())<0)
                    	...
                    	check = false;
                    	str = "The number inserted into the box X must be " +                    		  "\nin the range 0, " + (WithGraphEditor.maxWidth-1) + ". " +                    		  "\nThe number inserted into the box Y must be\n" +                    		  "in the range 0, " + (WithGraphEditor.maxHeight-1) + ".";
                    	JOptionPane.showMessageDialog(null,str,"Warning",JOptionPane.INFORMATION_MESSAGE);
                    }
                    
                    ElementoStato eps =  listaStato.getStartState();
                    
                    //controllo eventuale doppio start
                    if(eps!=null){
	                    if ((controllotipomessaggio.getSelectedType() == ElementoStato.START) && (eps!= processo)){
	                    	check = false;
	                    	str = "It is not possible to insert two Start States\n in the same diagram.";
	                    	JOptionPane.showMessageDialog(null,str,"Warning",JOptionPane.INFORMATION_MESSAGE);
	                    }
                    }
                    if (check){
                    	...
                    	processo.setType(controllotipomessaggio.getSelectedType());
                        ...
                        processo.setOnEntryCode(onEntryTextArea.getText());
                    	processografico.paintElementoGrafico(grafica2D);

						if(thrEditor.getListaStato().startExist())
							(thrEditor.getStateToolBar()).setEnabledStartButton(false);
						else
							(thrEditor.getStateToolBar()).setEnabledStartButton(true);
