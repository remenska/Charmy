package plugin.topologydiagram.file;
...
public class MenagementFileSemantica implements SerializableCharmyFile{
	...
	public SimpleValue getAttributeValue(SchemaEntry schemaEntry, String attributeName) {
		...
		if ((elementName.compareTo(Tag.identificatori)==0)&& (attributeName.compareTo(Tag.idProcessoStato)==0)){
			SimpleValue id =null;
			try {
				 id = new SimpleValue(plugin.topologydiagram.resource.data.ElementoProcessoStato.getNumIstanze());
			}
		...
		if ((elementName.compareTo(Tag.identificatori)==0)&& (attributeName.compareTo(Tag.idCanaleMessaggio)==0)){
			SimpleValue id =null;
			try {
				 id = new SimpleValue(plugin.topologydiagram.resource.data.ElementoCanaleMessaggio.getNumIstanze());
			}
	...
	public void resetForNewFile() {
		plugin.topologydiagram.resource.data.ElementoProcessoStato.setNumIstanze(0);
		plugin.topologydiagram.resource.data.ElementoCanaleMessaggio.setNumIstanze(0);
